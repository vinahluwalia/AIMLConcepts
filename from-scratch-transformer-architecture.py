...
- Positional Encoding
    - Component of the Transformer responsible for capturing information about the position of each token in the sequence
...
class InputEmbeddings(nn.Module):
    def __init__(self, vocab_size: int, d_model: int):
        ...
        # Embedding layer to map each token in the vocabulary to a d_model dimensional vector
        self.embedding = nn.Embedding(vocab_size, d_model)

    def forward(self, x):  # Calculates and returns the embeddings
        # Scaling the embeddings by square root of d_model ensures that token embeddings
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, max_seq_length):
        ...
        super().__init__()
        # Initialize a positional embedding -> Total number of sequences and dimensions of the model
        ...
        position = torch.arange(0, max_seq_length, dtype=torch.float).unsqueeze(
            1)  # Unsqueeze dim = 1, gives columnar shape
        ...
        self.pe = self.register_buffer('pe', pe.unsqueeze(0))
